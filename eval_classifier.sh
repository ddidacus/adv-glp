...
CONFIG="${1:-eval_config.yaml}"

PID_LIST=""
for gpu_id in 0 1 2 3; do
    echo "Launching eval_classifier on GPU $gpu_id"
    python eval_classifier.py run --config="$CONFIG" --gpu_id="$gpu_id" &
    PID_LIST+=" $!"
    sleep 1
done
...
python eval_classifier.py aggregate --out_dir="$(python -c "import yaml; print(yaml.safe_load(open('$CONFIG'))['out_dir'])")"
