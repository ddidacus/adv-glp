...
ALPHAS="${ALPHAS:-0.1}"
OUT_DIR="${OUT_DIR:-results/steering}"
...
for gpu_id in $(seq 0 $((NUM_GPUS - 1))); do
    echo "Launching steering on GPU $gpu_id"
    python scripts/steering.py run \
        --gpu_id="$gpu_id" \
        --num_gpus="$NUM_GPUS" \
        --model="$MODEL" \
...
python scripts/steering.py aggregate --out_dir="$OUT_DIR"
